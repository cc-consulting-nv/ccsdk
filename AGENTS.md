...
- **The per-profile `SessionStore` is the session carrier — never the refresh cookie.** One httpOnly cookie describes one session; on a cookie-first host a refresh for profile A returns the last-authenticated account's bearer. A regression here is an account-mixing bug, not just a failed request (`tests/sessionManager.test.js` → "a shared refresh cookie cannot hand one profile another account's session").
- **Hydrate with `ensureLiveSession()`, not `ready()`.** `ready()` is memoized per instance and only answers "did the first restore settle?". A background profile whose bearer was cleared, or that came up as a guest, stays hollow behind that settled promise. `hydrate()` pairs the two and is what `switchTo`, `remove`'s auto-switch, and the cross-tab follow all use.
- **`restoreStoredSession` must not let a refresh-only snapshot wipe a live in-memory bearer** (`mergeStoredSession` in `platformSdk.ts`). Refresh-cookie hosts omit the access token by design, not because the session ended.
